...
async function updateOffer(req: Request, pharmacyId: string, productId: string) {
...
export async function PUT(req: Request, { params }: { params: Promise<{ pharmacyId: string; productId: string }> }) {
  try {
    const { pharmacyId, productId } = await params;
    return await updateOffer(req, pharmacyId, productId);
  } catch (err) {
